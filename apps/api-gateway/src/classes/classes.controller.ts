...
export class ClassesController {
  ...
  @Get('hello')
  @ApiOperation({
    summary: 'Test classes service',
    description: 'Simple hello endpoint for testing classes service',
  })
  ...
  async createPost(@Body(ValidationPipe) postDto: PostsDTO) {
    try {
      const result = await this.classesService
        .send('posts.create', postDto)
        .pipe(
          timeout(5000),
          catchError((err) => {
  ...
  async create(@Body(ValidationPipe) createClassDto: CreateClassDto) {
    try {
      const result = await this.classesService
        .send('classes.create_class', createClassDto)
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  async findOne(@Param('class_id', ParseIntPipe) class_id: number) {
    try {
      return await this.classesService
        .send('classes.find_one', +class_id)
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  @ApiOperation({
    ...
    description: 'Remove a class by its ID',
  })
  @ApiParam({
    name: 'class_id',
    type: 'integer',
  ...
  async remove(@Param('class_id', ParseIntPipe) class_id: number) {
    try {
      const result = await this.classesService
        .send('classes.delete_class', +class_id)
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  @ApiResponse({ status: 404, description: 'Class not found' })
  @ApiResponse({ status: 408, description: 'Request timeout' })
  async getStudentsOfClass(@Param('class_id', ParseIntPipe) class_id: number) {
    try {
      const result = await this.classesService
        .send('classes.get_students_of_class', { class_id })
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  @ApiOperation({
    ...
    description: 'Enroll students in a class',
  })
  ...
  async addStudents(@Body() addStudentsDto: AddStudentsDto) {
    try {
      const result = await this.classesService
        .send('classes.add_students', addStudentsDto)
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  @ApiOperation({
    ...
    description: 'Unenroll a student from a class',
  })
  @ApiParam({
    name: 'class_id',
    type: 'integer',
  ...
  @ApiOperation({
    ...
    description: 'Enroll in a class using its code',
  })
  @ApiParam({
    name: 'class_code',
    type: 'string',
    description: 'Code of the class to join',
  })
  ...
  @ApiResponse({ status: 408, description: 'Request timeout' })
  async joinClass(
    @Param('class_code') class_code: string,
    @Body() data: { user_id: number },
  ) {
    try {
      const result = await this.classesService
        .send('classes.add_student_class_code', {
  ...
  @Get('of/:role/:id')
  @ApiOperation({
    summary: 'Get classes of a teacher or student',
    description: 'Retrieve classes by user role and ID',
  })
  @ApiParam({
    name: 'role',
    type: 'string',
  ...
  async getAllClassesOfTeacher(
    ...
    try {
      ...
      const result = await this.classesService
        .send(`classes.get_all_classes_of_${role}`, {
          user_id: id,
        })
        .pipe(
          timeout(5000),
  ...
  @ApiOperation({
    summary: 'Upload post with files in a class',
    description: 'Upload post with files for class',
  })
  @ApiParam({ name: 'id', type: 'integer', description: 'ID of the class' })
  @ApiResponse({
    status: 200,
    description: 'Messages for upload files successfully',
  })
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 408, description: 'Request timeout' })
  @UseInterceptors(FilesInterceptor('files'))
  async uploadPostWithFiles(
    @Param('id', ParseIntPipe) class_id,
    ...
    try {
      ...
      const result = await this.classesService
        .send('classes.upload_post_with_files', data)
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  @ApiOperation({
    summary: 'Upload file in a class',
    description: 'Upload file for class',
  })
  @ApiParam({ name: 'id', type: 'integer', description: 'ID of the class' })
  @ApiResponse({
    status: 200,
  ...
  async uplaodFiles(
    @Param('id', ParseIntPipe) class_id,
    @UploadedFiles() files: Express.Multer.File[],
    @Body('uploader_id', ParseIntPipe) uploader_id,
  ) {
    try {
      ...
      const result = await this.classesService
        .send('classes.upload_files', data)
        .pipe(
          timeout(5000),
          catchError((err) => {
            if (err instanceof TimeoutError) {
  ...
  @ApiResponse({
    status: 200,
    description: 'List of materials related to class',
  })
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 408, description: 'Request timeout' })
