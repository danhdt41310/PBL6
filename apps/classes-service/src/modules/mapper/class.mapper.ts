...
export class ClassMapper {
  /**
   * Helper method to recursively map post with materials and replies
   */
  private static mapPostWithMaterialsAndReplies(post: any): PostResponseDto {
    return {
      id: post.id,
      ...
      replies: post.replies
        ? post.replies.map((reply) =>
            this.mapPostWithMaterialsAndReplies(reply)
          )
        : [],
    };
  }
  ...
  static toResponseDto(
    classEntity: Class & { enrollments?: ClassEnrollment[] }
  ): ClassResponseDto {
    return {
      ...
      ...(classEntity.enrollments && {
        enrollments: classEntity.enrollments.map((enrollment) => ({
   ...
   */
  static toResponseAllInfoDto(
    classEntity: Class & { posts: Post[] }
  ): ClassResponseAllInfoDto {
    return {
      class_id: classEntity.class_id,
      class_name: classEntity.class_name,
      ...
      posts: classEntity.posts.map((post) =>
        this.mapPostWithMaterialsAndReplies(post)
      ),
    };
  }
   ...
   */
  static toDeleteClassResponseDto(class_id: number): {
    message: string;
    class_id: number;
  } {
    return {
   ...
   */
  static toClassListResponseDto(
    classes: (Class & { enrollments?: ClassEnrollment[] })[]
  ): ClassResponseDto[] {
   ...
   */
  static toClassListAllInfoResponseDto(
    classes: (Class & { posts: Post[] })[]
  ): ClassResponseAllInfoDto[] {
